...
export function SidebarHistory({ user }: { user: User | undefined }) {
  ...
  const handleDelete = () => {
    ...
    toast.success(t("chat.deleteAll", "Chat deleted"));
  };
      ...
      <SidebarGroup className="group-data-[collapsible=icon]:hidden">
        <SidebarGroupContent>
          <div className="flex w-full flex-row items-center justify-center gap-2 px-2 text-[13px] text-sidebar-foreground/60">
            {t("common.search", "Login to save and revisit previous chats!")}
          </div>
        </SidebarGroupContent>
      </SidebarGroup>
    );
  ...
  if (hasEmptyChatHistory) {
    return (
      <SidebarGroup className="group-data-[collapsible=icon]:hidden">
        ...
        <SidebarGroupContent>
          <div className="flex w-full flex-row items-center justify-center gap-2 px-2 text-[13px] text-sidebar-foreground/60">
            {t("chat.noChats", "Your conversations will appear here once you start chatting!")}
          </div>
        </SidebarGroupContent>
      </SidebarGroup>
    );
  ...
  return (
    <>
      ...
      <AlertDialog onOpenChange={setShowDeleteDialog} open={showDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("chat.areYouSure", "Are you absolutely sure?")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("chat.deleteConfirmationSingle", "This action cannot be undone. This will permanently delete your chat and remove it from our servers.")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
